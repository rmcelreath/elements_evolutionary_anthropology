...
\begin{tikzpicture}[
        ...
        \node[state] (CC) {CC};
		\node[state] (NN) [right of=CC] {NN};
        
		\path[->] (CC) edge[loop left] node {$(1-x)^2 + x^2$} (CC);
		\path[->] (CC) edge[bend left=25] node[above] {$2x(1-x)$} (NN);
		\path[->] (NN) edge[bend left=25] node[below] {$1$} (CC);
    \end{tikzpicture}
...
\section*{Variance reduction and risk pooling}
...
Human societies build absurd and amazing institutions on top of hostages. In medieval Europe, royalty dominated political power. And royalty is a family. But royal families routinely raised one another's children, literal royal hostages, to ensure that they could remain at peace. ``Don't invade my lands,'' the princess cries, ``or your princess gets it!''\footnote{Peacemaking in Middle Ages citation}  %The mafia is sometimes called a ``family,'' but no human family is that large. More accurately a mafia uses family ties to bind people to the organization. 
...
\vspace{-6pt}
\figmarlab{figK1}
%%%%% plot of resources against fitness change
\begin{center}
\begin{tikzpicture}
\begin{axis}[
    	axis lines=middle,
        ...
        y label style={at={(current axis.north west)},above=0.1mm},
    	samples=50]
	\addplot[black,thick,domain=0:10] {1*x^0.8};
	
	\addplot +[mark=none,thin,dashed,black] coordinates {(3, -1) (3, 3^0.8)};
	\addplot +[mark=none,thin,dashed,black] coordinates {(-1, 3^0.8) (3, 3^0.8)};

	\addplot +[mark=none,thin,dashed,black] coordinates {(9, -1) (9, 9^0.8)};
	\addplot +[mark=none,thin,dashed,black] coordinates {(-1, 9^0.8) (9, 9^0.8)};

  	\addplot[bemphcol,thick,domain=0:10] {1*x};
	\node[bemphcol] at (axis cs: 5,8) {Additive};
	\node[black] at (axis cs: 6.5,2) {Diminishing};
	%\node[black] at (-1,2.5) {$\frac{b}{b-c}$};
\end{axis}
\end{tikzpicture}
\end{center}
...
\section*{Reproductive skew}
x

\section*{What is inclusive fitness and does selection maximize it?}
x
...
\section*{Local competition}

Kinship can be a powerful source of assortment. But that assortment does not always lead to more cooperation. It really depends upon how families interact and the broader population structure, and even the life cycle of the organism. 

Let's consider two examples. The first asks whether offspring should disperse away from where they were born. The second asks about how many daughters and sons a mother should produce, if she could choose.

\subsection*{Dispersal}

Suppose a environment in which there are $n$ sites where an adult can mature and reproduce. Only one adult can reside at each site, so the adult population is at most size $n$. Each adult produces on average $k$ offspring and then dies. A proportion $v$ of offspring disperse to another random site. A proportion $1-v$ remain at their natal site. Dispersal is dangerous, and only a proportion $p$ of dispersers survive to arrive at another site. Whether at a natal site or a new site, all offspring at a site compete to mature, and only one can succeed.

If natural selection could adjust $v$, which value would it end up at? To answer that question, we need to find a value of $v$ that is evolutionarily stable. This means that a mutant with a similar value of $v$ could not grow faster and replace a common-type $v$. So suppose a population in which $n-1$ sites have resident adults with dispersal value $v$. But one site has a mutant, and the mutant programs $v+\delta$ of its offspring to disperse, where $\delta$ is small.
...
\begin{tikzpicture}[
        ...
        \node[state] (home) {C};
		\node[] (a) [draw=white,left of=home] {$vpk$};
		\node[] (b) [draw=white,above left of=home] {$(n-2)\dfrac{vpk}{n-1}$};
		\node[] (c) [draw=white,above right of=home] {$\dfrac{(v+\delta)pk}{n-1}$};
		%\node[state] (NN) [right of=CC] {NN};
        
		\path[->] (home) edge[loop below] node {$(1-v)k$} (home);
		\path[->] (home) edge[bend left=25] node[above] {dispersers} (a);
		\path[->] (b) edge[bend left=25] node[left] {Common-type immigrants} (home);
		\path[->] (c) edge[bend right=25] node[right] {~Mutant immigrants} (home);
		%\path[->] (NN) edge[bend left=25] node[below] {$1$} (CC);
    \end{tikzpicture}
...
All of the offspring at this site compete for it. So the probability the winner is a mutant offspring is:
\begin{align*}
	\Pr(M|C) = (n-1) \times \frac{\dfrac{(v+\delta)pk}{n-1}}{(1-v)k + \dfrac{(v+\delta)pk}{n-1} + \dfrac{(n-2)vpk}{n-1} }
\end{align*}
The numerator is the expected number of mutant arrivals. The denominator is the sum of all arrivals and non-dispersing offspring. This is for only one common-type site. There are $n-1$ such sites. That's why there is an $n-1$ in front. This expression can be simplified. But we'll worry about that later.

The expression above is only part of a mutant's fitness. We also need to know the probability a mutant offspring wins the natal site. Again a diagram to help:

\vspace{-6pt}
\figmarlab{figKdispersal2}
\tikzset{every loop/.style={min distance=7mm,looseness=10}}
\begin{center}
\begin{tikzpicture}[
            > = stealth, % arrow head style
        ...
        \node[state] (home) {M};
		\node[] (a) [draw=white,left of=home] {$(v+\delta)pk$};
		\node[] (b) [draw=white,above left of=home] {$(n-1)\dfrac{vpk}{n-1}$};
		%\node[state] (NN) [right of=CC] {NN};
        
		\path[->] (home) edge[loop below] node {$(1-v-\delta)k$} (home);
		\path[->] (home) edge[bend left=25] node[above] {dispersers} (a);
		\path[->] (b) edge[bend left=25] node[right] {Common-type immigrants} (home);
		%\path[->] (NN) edge[bend left=25] node[below] {$1$} (CC);
    \end{tikzpicture}

	\vspace{6pt}
	\emph{A mutant site: How soft your fields so green}
	\vspace{6pt}
\end{center}
...
%\begin{mathbox}{b}{Fisherian algebra.}
%We want to simplify:
%\begin{align*}
%	2(1-r) > 1-r^\prime +  r^\prime \frac{1-r}{r} 
%\end{align*}
%\end{mathbox}
